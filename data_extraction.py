...
def build_player_pool(n_tbase, seed):
    ...
    players = [RolloutPlayer(n_rollouts=n+1) for n in range(20)]
...
def collect_data(players, rounds=200000, seed=92122):
    ...
    for i in tqdm(range(rounds), desc="Collecting play data"):
        ...
        for begin in range(2):
            ...
            if rng.random() > 0.5 and player1 is not RandomPlayer and player2 is not RandomPlayer:
...
X_all, y_all = collect_data(players=player_pool, rounds=5000)
X_np = np.array(X_all, dtype=np.float32)
y_np = np.array(y_all, dtype=np.int64)
counts = np.bincount(y_np, minlength=7)
